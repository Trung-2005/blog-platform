...
| Tính năng | Mô tả |
|---|---|
| **Danh mục lồng nhau** | Menu dropdown phân cấp (Ghi Chép, Nhật Ký, Truyện, Thơ) |
...
| **About + Copyright** | Các trang tĩnh |
...
sequenceDiagram
    ...
    User->>Vue: Truy cập /nhat-ky/bai-viet-1
...
| Danh mục Nhật Ký | `/nhat-ky` → `CategoryView.vue` | `GET /api/posts.php?category=nhat-ky` |
| Chi tiết bài | `/2020/03/17/slug` → `PostView.vue` | `GET /api/posts.php?slug=...` |
| Theo tag | `/tag/ten-tag` → `TagView.vue` | `GET /api/posts.php?tag=ten-tag` |
